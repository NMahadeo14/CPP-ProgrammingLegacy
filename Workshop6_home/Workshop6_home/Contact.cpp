...
namespace sict
{
	...
	Contact& Contact::operator=(const Contact& src)
	{
		...
		strncpy(m_name, src.m_name, max_name_size - 1);

		if (this != &src)
		{
			this->m_numofPhone = src.m_numofPhone;
			delete [] m_phone;
			if (src.m_phone != nullptr)
			{
				this->m_phone = new long long[this->m_numofPhone];
				for (int i = 0; i < m_numofPhone; i++)
	...
	{
		if (isValid(newPhone) && isEmpty() == true)
		{
			Contact a(m_name, m_phone, m_numofPhone);

			delete[] m_phone;
			m_phone = nullptr;
			m_numofPhone++;

			m_phone = new long long[m_numofPhone];
