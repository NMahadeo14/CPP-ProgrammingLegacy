...
namespace ama
{
	Product::Product(char Producttype) : m_productType('N')
	{
	...
	{
		if (strcmp(this->m_ProductName, rhsProduct.m_ProductName) > 0)
		{
			return true;
		}
		else
	...
	std::istream& Product::read(std::istream& in, bool interractive)
	{
		...
		else
		{
			...
			*this = Product(sku, name, unit, price, quantityNeeded, quantityavailable, StatusTax);
		}
